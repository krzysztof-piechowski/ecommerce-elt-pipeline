...
try:
    ...
    logger.error(f"Azure connection failed. Critical for GHA execution.\nError: {e}")
    raise e
...
def upload_json_to_azure(folder, filename, data):
    """Uploads data to Azure Blob Storage in JSON format"""
    if not blob_service_client:
        logger.warning(f"Skipping upload for {filename} (No Azure connection).")
        return
    ...
    try:
        ...
        logger.warning(f"Upload failed for {filename}: {e}")
...
# Base Date is always NOW for new data
...
# Upload Products (Master Data) - Always uploads (deduplicated downstream)
products_ts = datetime.now().strftime("%Y%m%d_%H_%M_%S")
logger.info(f"Uploading Product Catalog ({len(products)} items)...")
upload_json_to_azure("products_raw", f"products_{products_ts}.json", products)
...
for batch in range(1, BATCHES + 1):
    ...
    # --- UPLOAD BATCH (IN-MEMORY) ---
    upload_json_to_azure("customers_raw", f"customers_{timestamp_str}.json", batch_customers)
    upload_json_to_azure("addresses_raw", f"addresses_{timestamp_str}.json", batch_addresses)
    upload_json_to_azure("orders_raw", f"orders_{timestamp_str}.json", batch_orders)
    upload_json_to_azure("order_items_raw", f"items_{timestamp_str}.json", batch_items)
    upload_json_to_azure("payments_raw", f"payments_{timestamp_str}.json", batch_payments)
    upload_json_to_azure("shipments_raw", f"shipments_{timestamp_str}.json", batch_shipments)
    upload_json_to_azure("order_status_history_raw", f"history_{timestamp_str}.json", batch_history)

    time.sleep(1)

# ==========================================
# SAVE STATE (Updated IDs)
# ==========================================
new_state = {
...
logger.info(f"Data generation finished. Next Order ID will be: {order_id_seq}")
